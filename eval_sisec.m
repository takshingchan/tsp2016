...
s1 = load_audio(fullfile(source,'bass.wav'),true,false,false);
s2 = repmat(s1,[1,3-size(s1,2)]);
s1 = load_audio(fullfile(source,'drums.wav'),true,false,false);
s2 = s2+repmat(s1,[1,3-size(s1,2)]);
s1 = load_audio(fullfile(source,'other.wav'),true,false,false);
s2 = s2+repmat(s1,[1,3-size(s1,2)]);
s1 = load_audio(fullfile(source,'vocals.wav'),true,false,false);
